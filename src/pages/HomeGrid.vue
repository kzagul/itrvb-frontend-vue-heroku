        ...
        <template>
            <v-dialog v-model="showInfo" max-width="500px">
                    <v-card>
                        ...
                        <v-card-subtitle class="text-h5" > {{currentCartClassificationNumber}}</v-card-subtitle>

                    


                        <v-row class="justify-end align-end mx-16" >
                            <p class="font-weight-bold mx-4" v-text="card.fk_director_id"></p>
                            <p class="font-weight-bold mx-4" v-text="card.fk_contacts_id"></p>
                        </v-row>
...
export default {
  data(){
    return {
        ...
        // current variables
        currentCartTitle: '',
        currentCartClassificationNumber: '',

        imageSrc1: 'https://sportgyms.ru/uploads/posts/2016-06/1464857702_02.jpg',
    ...
    methods: {
        showInfoItem(card) {
            ...
            this.currentCartClassificationNumber = this.cards[this.selectedItemIndex].classificationnumber
            this.showInfo = true
        },
        closeInfoItem () {
            this.$nextTick(() => {
